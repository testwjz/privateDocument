...
public class HTTPHC4Impl extends HTTPHCAbstractImpl {
    ...
    protected void setupRequest(URL url, HttpRequestBase httpRequest, HTTPSampleResult res)
        ...
        //signatureVerifiy begin zhangwenjun
        if (JMeterUtils.getProperty("gateway_is_available").equals("True")) {
            setHttpHeaderDefault(url, httpRequest.getMethod(), httpRequest);
        }

        //signatureVerifiy end zhangwenjun
    ...
    private void setHttpHeaderDefault(URL url, String method, HttpRequestBase httpRequest) throws IOException {
        String entityBody = "";
        if (method.equals("POST")) {
            entityBody = setupHttpEntityEnclosingRequestData((HttpEntityEnclosingRequestBase) httpRequest);
//            if (getSendParameterValuesAsPostBody()) {
//                System.out.println("parameter");
//                entityBody = DGGSignatureVerifiy.getFormStr(DGGSignatureVerifiy.createSignParameter(entityBody));
//            }
        } else if (method.equals("GET")) {
            entityBody = DGGSignatureVerifiy.getGetParamStr(DGGSignatureVerifiy.getGetParamObj(url.toString()));
        } else {
            return;
        }
//        String path_route = url.getPath().split("/")[1];
        String sysCode = JMeterUtils.getProperty("dgg_sysCode");
        String secret = JMeterUtils.getProperty("dgg_secret");
        SortedMap<Object, Object> signMap = new TreeMap();
        String timeString = String.valueOf(Instant.now().toEpochMilli());
        signMap.put("time", timeString);
        String nonce = UUID.randomUUID().toString();
        signMap.put("nonce", nonce);
        signMap.put("data", entityBody);
        String signResultStr = DGGSignatureVerifiy.createSign(signMap, sysCode, secret);
        httpRequest.setHeader("time", timeString);
        httpRequest.setHeader("nonce", nonce);
        httpRequest.setHeader("sysCode", sysCode);
        httpRequest.setHeader("sign", signResultStr);
        httpRequest.setHeader("replaceEmoji", "true");
        log.debug("zhangwenjun-signResultStr:" + signResultStr);
    }
